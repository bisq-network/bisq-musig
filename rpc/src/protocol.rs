...
impl TradeModelStore for TradeModelMemoryStore {
    fn add_trade_model(&self, trade_model: TradeModel) {
        ...
        self.lock().unwrap().insert(
            trade_model.trade_id.clone(),
            Arc::new(Mutex::new(trade_model)),
        );
    }
...
pub static TRADE_MODELS: LazyLock<TradeModelMemoryStore> =
...
#[expect(
    ...
    reason = "not sure removing common postfix would make things clearer"
...
impl RedirectionReceiver<NetworkUnchecked> {
    fn require_network(self, required: Network) -> Result<RedirectionReceiver> {
        Ok(RedirectionReceiver {
            address: self.address.require_network(required)?,
            amount: self.amount,
        })
    }
}
...
impl TradeModel {
    ...
    pub const fn set_peer_key_shares(
        &mut self,
        buyer_output_pub_key: Point,
        seller_output_pub_key: Point,
    ) {
        self.buyer_output_key_ctx.peers_key_share =
            Some(KeyPair::from_public(buyer_output_pub_key));
        self.seller_output_key_ctx.peers_key_share =
            Some(KeyPair::from_public(seller_output_pub_key));
        if self.am_buyer() {
    ...
    pub fn init_my_fee_bump_addresses(&mut self) -> Result<()> {
        ...
        if self.am_buyer() {
            self.buyers_warning_tx_fee_bump_address = Some(
                "tb1pkar3gerekw8f9gef9vn9xz0qypytgacp9wa5saelpksdgct33qdqs257jl"
                    .parse::<Address<_>>()?
                    .require_network(Network::Signet)?,
            );
            self.buyers_redirect_tx_fee_bump_address = Some(
                "tb1pv537m7m6w0gdrcdn3mqqdpgrk3j400yrdrjwf5c9whyl2f8f4p6qg5eh2l"
                    .parse::<Address<_>>()?
                    .require_network(Network::Signet)?,
            );
        } else {
            self.sellers_warning_tx_fee_bump_address = Some(
                "tb1pzvynlely05x82u40cts3znctmvyskue74xa5zwy0t5ueuv92726s0cz8g8"
                    .parse::<Address<_>>()?
                    .require_network(Network::Signet)?,
            );
            self.sellers_redirect_tx_fee_bump_address = Some(
                "tb1pt5xd4aqe9whmvlz78mt39rvdlgpp6hujs5ggwan5285zjnsf73rq8kunpq"
                    .parse::<Address<_>>()?
                    .require_network(Network::Signet)?,
            );
        }
    ...
    pub fn get_my_fee_bump_addresses(&self) -> Option<[&Address; 2]> {
        Some(if self.am_buyer() {
            [
                self.buyers_warning_tx_fee_bump_address.as_ref()?,
                self.buyers_redirect_tx_fee_bump_address.as_ref()?,
            ]
        } else {
            [
                self.sellers_warning_tx_fee_bump_address.as_ref()?,
                self.sellers_redirect_tx_fee_bump_address.as_ref()?,
            ]
        })
    }

    pub fn set_peer_fee_bump_addresses(
        &mut self,
        fee_bump_addresses: [Address<NetworkUnchecked>; 2],
    ) -> Result<()> {
        ...
        let (warning_tx_fee_bump_address, redirect_tx_fee_bump_address) = fee_bump_addresses
            .map(|addr| addr.require_network(Network::Signet))
            .into();
        if self.am_buyer() {
    ...
    where
        I: IntoIterator<Item = Result<RedirectionReceiver<NetworkUnchecked>, E>>,
    ...
    pub fn get_my_nonce_shares(&self) -> Option<ExchangedNonces<ByRef>> {
        Some(ExchangedNonces {
            swap_tx_input_nonce_share: &(self
                .swap_tx_input_sig_ctx
                .my_nonce_share
                .as_ref()?
                .pub_nonce),
            buyers_warning_tx_buyer_input_nonce_share: &(self
                .buyers_warning_tx_buyer_input_sig_ctx
                .my_nonce_share
                .as_ref()?
                .pub_nonce),
            buyers_warning_tx_seller_input_nonce_share: &(self
                .buyers_warning_tx_seller_input_sig_ctx
                .my_nonce_share
                .as_ref()?
                .pub_nonce),
            sellers_warning_tx_buyer_input_nonce_share: &(self
                .sellers_warning_tx_buyer_input_sig_ctx
                .my_nonce_share
                .as_ref()?
                .pub_nonce),
            sellers_warning_tx_seller_input_nonce_share: &(self
                .sellers_warning_tx_seller_input_sig_ctx
                .my_nonce_share
                .as_ref()?
                .pub_nonce),
            buyers_redirect_tx_input_nonce_share: &(self
                .buyers_redirect_tx_input_sig_ctx
                .my_nonce_share
                .as_ref()?
                .pub_nonce),
            sellers_redirect_tx_input_nonce_share: &(self
                .sellers_redirect_tx_input_sig_ctx
                .my_nonce_share
                .as_ref()?
                .pub_nonce),
        })
    ...
    pub const fn set_peer_nonce_shares(&mut self, peer_nonce_shares: ExchangedNonces<ByVal>) {
        ...
        self.buyers_warning_tx_seller_input_sig_ctx
            .peers_nonce_share = Some(peer_nonce_shares.buyers_warning_tx_seller_input_nonce_share);
        self.sellers_warning_tx_buyer_input_sig_ctx
            .peers_nonce_share = Some(peer_nonce_shares.sellers_warning_tx_buyer_input_nonce_share);
    ...
    pub fn sign_partial(&mut self) -> Result<()> {
        ...
        let [buyer_key_ctx, seller_key_ctx] =
            [&self.buyer_output_key_ctx, &self.seller_output_key_ctx];
    ...
    pub fn get_my_partial_signatures_on_peer_txs(&self) -> Option<ExchangedSigs<ByRef>> {
        Some(if self.am_buyer() {
            ExchangedSigs {
                peers_warning_tx_buyer_input_partial_signature: self
                    .sellers_warning_tx_buyer_input_sig_ctx
                    .my_partial_sig
                    .as_ref()?,
                peers_warning_tx_seller_input_partial_signature: self
                    .sellers_warning_tx_seller_input_sig_ctx
                    .my_partial_sig
                    .as_ref()?,
                peers_redirect_tx_input_partial_signature: self
                    .sellers_redirect_tx_input_sig_ctx
                    .my_partial_sig
                    .as_ref()?,
                swap_tx_input_partial_signature: Some(
                    self.swap_tx_input_sig_ctx.my_partial_sig.as_ref()?,
                ),
            }
        } else {
            ExchangedSigs {
    ...
    pub const fn set_peer_partial_signatures_on_my_txs(&mut self, sigs: &ExchangedSigs<ByVal>) {
        if self.am_buyer() {
            self.buyers_warning_tx_buyer_input_sig_ctx.peers_partial_sig =
                Some(sigs.peers_warning_tx_buyer_input_partial_signature);
            self.buyers_warning_tx_seller_input_sig_ctx
                .peers_partial_sig = Some(sigs.peers_warning_tx_seller_input_partial_signature);
            self.buyers_redirect_tx_input_sig_ctx.peers_partial_sig =
                Some(sigs.peers_redirect_tx_input_partial_signature);
        } else {
            self.sellers_warning_tx_buyer_input_sig_ctx
                .peers_partial_sig = Some(sigs.peers_warning_tx_buyer_input_partial_signature);
            self.sellers_warning_tx_seller_input_sig_ctx
                .peers_partial_sig = Some(sigs.peers_warning_tx_seller_input_partial_signature);
            self.sellers_redirect_tx_input_sig_ctx.peers_partial_sig =
                Some(sigs.peers_redirect_tx_input_partial_signature);
        }
    ...
    pub fn aggregate_partial_signatures(&mut self) -> Result<()> {
        if self.am_buyer() {
            self.buyers_warning_tx_buyer_input_sig_ctx
                .aggregate_partial_signatures(&self.buyer_output_key_ctx)?;
            self.buyers_warning_tx_seller_input_sig_ctx
                .aggregate_partial_signatures(&self.seller_output_key_ctx)?;
            self.buyers_redirect_tx_input_sig_ctx
                .aggregate_partial_signatures(&self.buyer_output_key_ctx)?;
    ...
    pub fn aggregate_swap_tx_partial_signatures(&mut self) -> Result<()> {
        ...
        self.swap_tx_input_sig_ctx
            .aggregate_partial_signatures(my_key_ctx)?;
        Ok(())
    }
    ...
    pub fn set_peer_private_key_share_for_my_output(
        &mut self,
        prv_key_share: Scalar,
    ) -> Result<()> {
        self.get_my_key_ctx_mut()
            .peers_key_share
            .as_mut()
    ...
    pub fn recover_seller_private_key_share_for_buyer_output(
        &mut self,
        swap_tx_input_signature: &LiftedSignature,
    ) -> Result<()> {
        let adaptor_sig = self
            .swap_tx_input_sig_ctx
            .aggregated_sig
            .ok_or(ProtocolErrorKind::MissingAggSig)?;
        let adaptor_secret: MaybeScalar = adaptor_sig
            .reveal_secret(swap_tx_input_signature)
            .ok_or(ProtocolErrorKind::MismatchedSigs)?;
        self.buyer_output_key_ctx
            .set_sellers_prv_key_if_buyer(adaptor_secret.try_into()?)
    }
}
...
impl KeyPair {
    ...
    fn from_private(prv_key: Scalar) -> Self {
        Self {
            pub_key: prv_key.base_point_mul(),
            prv_key,
        }
    }
}
...
impl NoncePair {
    fn new(nonce_seed: impl Into<NonceSeed>, aggregated_pub_key: Point) -> Self {
        let sec_nonce = SecNonceBuilder::new(nonce_seed)
            ...
            .build();
        Self {
            pub_nonce: sec_nonce.public_nonce(),
...
impl KeyCtx {
    fn init_my_key_share(&mut self) -> &KeyPair {
        ...
        self.my_key_share
            .insert(KeyPair::random(&mut rand::thread_rng()))
    }

    fn get_key_shares(&self) -> Option<[Point; 2]> {
        Some(if self.am_buyer {
            [
                self.my_key_share.as_ref()?.pub_key,
                self.peers_key_share.as_ref()?.pub_key,
            ]
        } else {
            [
                self.peers_key_share.as_ref()?.pub_key,
    ...
    fn get_prv_key_shares(&self) -> Option<[Scalar; 2]> {
        Some(if self.am_buyer {
            [
                self.my_key_share.as_ref()?.prv_key,
...
impl SigCtx {
    fn init_my_nonce_share(&mut self, key_ctx: &KeyCtx) -> Result<()> {
        let aggregated_pub_key = key_ctx
            .aggregated_key
            .as_ref()
            .ok_or(ProtocolErrorKind::MissingAggPubKey)?
            .pub_key;
        // TODO: Make the RNG configurable, to aid unit testing:
        self.my_nonce_share = Some(NoncePair::new(
    ...
    fn get_nonce_shares(&self) -> Option<[&PubNonce; 2]> {
        Some(if self.am_buyer {
            [
                &self.my_nonce_share.as_ref()?.pub_nonce,
                self.peers_nonce_share.as_ref()?,
            ]
        } else {
            [
                self.peers_nonce_share.as_ref()?,
    ...
    fn aggregate_nonce_shares(&mut self) -> Result<&AggNonce> {
        let agg_nonce = AggNonce::sum(
            self.get_nonce_shares()
                .ok_or(ProtocolErrorKind::MissingKeyShare)?,
        );
        if matches!(
            (&agg_nonce.R1, &agg_nonce.R2),
            (MaybePoint::Infinity, MaybePoint::Infinity)
        ) {
    ...
    fn sign_partial(&mut self, key_ctx: &KeyCtx, message: Vec<u8>) -> Result<&PartialSignature> {
        ...
        let aggregated_nonce = &self
            .aggregated_nonce
            .as_ref()
            .ok_or(ProtocolErrorKind::MissingAggNonce)?;

        let sig = musig2::adaptor::sign_partial(
            key_agg_ctx,
            seckey,
            secnonce,
            aggregated_nonce,

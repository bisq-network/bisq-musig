...
impl musig_server::Musig for MusigImpl {
    ...
    async fn init_trade(
        &self,
        request: Request<PubKeySharesRequest>,
    ) -> Result<Response<PubKeySharesResponse>> {
        handle_request(request, move |request| {
            let mut trade_model =
                TradeModel::new(request.trade_id, request.my_role.try_proto_into()?);
            trade_model.init_my_key_shares();
            let my_key_shares = trade_model
                .get_my_key_shares()
    ...
    #[instrument(skip_all)]
    async fn get_nonce_shares(
        &self,
        request: Request<NonceSharesRequest>,
    ) -> Result<Response<NonceSharesMessage>> {
        handle_musig_request(request, move |request, trade_model| {
            ...
            trade_model.buyers_security_deposit =
                Some(Amount::from_sat(request.buyers_security_deposit));
            trade_model.sellers_security_deposit =
                Some(Amount::from_sat(request.sellers_security_deposit));
            trade_model.deposit_tx_fee_rate =
                Some(FeeRate::from_sat_per_kwu(request.deposit_tx_fee_rate));
            trade_model.prepared_tx_fee_rate =
                Some(FeeRate::from_sat_per_kwu(request.prepared_tx_fee_rate));
            trade_model.init_my_fee_bump_addresses()?;
            trade_model.init_my_nonce_shares()?;

            let my_fee_bump_addresses = trade_model
                .get_my_fee_bump_addresses()
                .ok_or_else(|| Status::internal("missing fee bump addresses"))?;
            let my_nonce_shares = trade_model
                .get_my_nonce_shares()
    ...
    async fn get_partial_signatures(
        &self,
        request: Request<PartialSignaturesRequest>,
    ) -> Result<Response<PartialSignaturesMessage>> {
        handle_musig_request(request, move |request, trade_model| {
            let peer_nonce_shares = request
                .peers_nonce_shares
            ...
            trade_model.set_redirection_receivers(
                request
                    .receivers
                    .into_iter()
                    .map(TryProtoInto::try_proto_into),
            )?;
    ...
    async fn sign_deposit_tx(
        &self,
        request: Request<DepositTxSignatureRequest>,
    ) -> Result<Response<DepositPsbt>> {
        handle_musig_request(request, move |request, trade_model| {
            let peers_partial_signatures = request
                .peers_partial_signatures
                .ok_or_else(|| Status::not_found("missing request.peers_partial_signatures"))?;
            trade_model
                .set_peer_partial_signatures_on_my_txs(&peers_partial_signatures.try_proto_into()?);
    ...
    #[instrument(skip_all)]
    async fn publish_deposit_tx(
        &self,
        request: Request<PublishDepositTxRequest>,
    ) -> Result<Response<Self::PublishDepositTxStream>> {
    ...
    #[instrument(skip_all)]
    async fn subscribe_tx_confirmation_status(
        &self,
        request: Request<SubscribeTxConfirmationStatusRequest>,
    ...
    #[instrument(skip_all)]
    async fn sign_swap_tx(
        &self,
        request: Request<SwapTxSignatureRequest>,
    ) -> Result<Response<SwapTxSignatureResponse>> {
        handle_musig_request(request, move |request, trade_model| {
            ...
            let prv_key_share = trade_model
                .get_my_private_key_share_for_peer_output()
    ...
    async fn close_trade(
        &self,
        request: Request<CloseTradeRequest>,
    ) -> Result<Response<CloseTradeResponse>> {
        handle_musig_request(request, move |request, trade_model| {
            if let Some(peer_prv_key_share) =
                request.my_output_peers_prv_key_share.try_proto_into()?
            {
                ...
                trade_model
                    .recover_seller_private_key_share_for_buyer_output(&swap_tx_input_signature)?;
                trade_model.aggregate_private_keys_for_my_output()?;
            } else {
            ...
            let my_prv_key_share = trade_model
                .get_my_private_key_share_for_peer_output()
                .ok_or_else(|| Status::internal("missing private key share"))?;
...
fn mock_tx_confirmation_status_stream(
    trade_id: String,
) -> impl Stream<Item = Result<TxConfirmationStatus>> {
    ...
    .on_drop(move || {
        debug!(
            trade_id,
            "Deposit tx status confirmation stream has been dropped."
        )
    })
}
...
impl wallet_server::Wallet for WalletImpl {
    ...
    ) -> Result<Response<WalletBalanceResponse>> {
    ...
    ) -> Result<Response<NewAddressResponse>> {
    ...
    ) -> Result<Response<ListUnspentResponse>> {
        handle_request(request, |_request| {
    ...
    #[instrument(skip_all)]
    async fn register_confidence_ntfn(
        &self,
        request: Request<ConfRequest>,
    ) -> Result<Response<Self::RegisterConfidenceNtfnStream>> {
        handle_request(request, move |request| {
            let txid = request.tx_id.try_proto_into()?;
            let conf_events = self
                .wallet_service
                .get_tx_confidence_stream(txid)
...
{
    handle_request(request, move |request| {
        let trade_model = TRADE_MODELS
            .get_trade_model(request.trade_id())
            .ok_or_else(|| {
                Status::not_found(format!("missing trade with id: {}", request.trade_id()))
            })?;
    ...
    F: FnOnce(Req) -> Result<Res>,
{
    ...
    let response = handler(request.into_inner()).inspect_err(|e| error!("Error response: {e}"))?;

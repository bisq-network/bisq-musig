...
mod tests {
    ...
    fn test_swap() -> anyhow::Result<()> {
        ...
        assert!(
            bob.p_tik.agg_sec.is_some(),
            "We should have the aggregated secret key now"
        );
        assert!(
            bob.p_tik.other_sec.unwrap() == alice.p_tik.sec,
    ...
    fn test_claim_too_early() -> anyhow::Result<()> {
        ...
        match rtx {
            Ok(_) => panic!(
                "ClaimTx should not go through, because its been broadcast too early.
...
fn test_q_tik() -> anyhow::Result<()> {
    ...
    let seckeys: Vec<musig2::secp::Scalar> = if &alice.q_tik.pub_point < &bob.q_tik.pub_point {
    ...
    let signature_secp = bitcoin::taproot::Signature {
        signature: sig1,
        sighash_type,
    };
    ...
    assert_eq!(
        &newaggkey,
        &newctx2.aggregated_pubkey_untweaked(),
        "newaggkey not equal"
    );
    ...
    assert_eq!(
        &d.serialize(),
        &tweaked.to_inner().public_key().serialize(),
        "pubkey not equal"
    );
